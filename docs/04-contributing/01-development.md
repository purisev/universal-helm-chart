...
| `bash .github/scripts/check-examples.sh` | After touching the values shape, the schema or anything under `docs/02-examples/` — renders every example, including the values inlined in its Argo CD and Flux manifests. Needs `yq`. |
...
- **Changed an example?** Each example carries the same values three times: `values.yaml`, the Argo CD `Application`, and the Flux `HelmRelease`. Change one and the other two go stale. `check-examples.sh` renders all three and fails on the ones that no longer install.
