...
- **Workload has a main Service** (`service.enabled: true` and at least one entry in `service.ports`): append a `metrics` port to the Service's `ports[]` and a matching `containerPort` to the workload's main container. Sidecars, init containers and job containers never get it — `ContainerPort.name` has to be unique within a Pod, and they do not serve the endpoint. Port name is fixed as `metrics` (Kubernetes convention).
- **Workload has no main Service**: emit a **dedicated** Service with only the metrics port. Name: `<release-fullname>-<workload>-metrics`. On a StatefulSet the governing headless Service takes the port instead whenever the chart renders it; the dedicated Service appears only when neither Service exists. This Service is also picked up by the ServiceMonitor / VMServiceScrape (when CRD discovery + service target is selected — see [ADR 008](008-multi-provider-monitoring.md)).
