...
| Lists (`envSecrets`, `envConfigMaps`, `tolerations`, `hostAliases`) | **Concat** — earlier-layer entries appear first, later-layer entries appended. Duplicates are not deduplicated; map-keyed inheritance (`inherit.configMapMount.<name>: false`) is the way to suppress one. |
...
The opt-out is expressed at the inheriting layer via `inherit.*` flags, not at the producing layer:
